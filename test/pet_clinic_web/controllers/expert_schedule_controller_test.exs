defmodule PetClinicWeb.ExpertScheduleControllerTest do
  ...
  describe "create expert_schedule" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.expert_schedule_path(conn, :create), expert_schedule: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.expert_schedule_path(conn, :create), expert_schedule: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Expert schedule"
    end
  end
  ...
  describe "edit expert_schedule" do
    ...
    test "renders form for editing chosen expert_schedule", %{conn: conn, expert_schedule: expert_schedule} do
  ...
  describe "update expert_schedule" do
    ...
    test "redirects when data is valid", %{conn: conn, expert_schedule: expert_schedule} do
      conn = put(conn, Routes.expert_schedule_path(conn, :update, expert_schedule), expert_schedule: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, expert_schedule: expert_schedule} do
      conn = put(conn, Routes.expert_schedule_path(conn, :update, expert_schedule), expert_schedule: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Expert schedule"
    end
  end
